...
public class MainApp extends Application {
	...
	public void start(Stage stagePrincipal) 
	{
        ...
        File file2 = getRepresentantFilePath();
        if (file2 != null)
        {
            loadRepresentantDataFromFile(file2);
        }
        
        File file3 = getProspectFilePath();
        if (file3 != null)
        {
            loadProspectDataFromFile(file3);
        }
	...
	// return le fichier preference comme il était a la precedente ouverture, si non trouver retourne null
		...
		{
		    Preferences prefs = Preferences.userNodeForPackage(MainApp.class);
		    String filePath = prefs.get("representant", null);
	...
	// met en place le chemin du fichier ou l'enleve si fichier = null
		public void setRepresentantFilePath(File file) 
		{
		    ...
		    if (file != null) 
		    {
		        prefs.put("representant", file.getPath());
	...
	public void savePersonDataToFile(File file, File file2, File file3) 
	{
	    try
	    {
	        JAXBContext context = JAXBContext.newInstance(ProspectListWrapper.class);
	        ...
	        m2.marshal(wrapper3, file2);
	        m3.marshal(wrapper2, file3);

	        // Save the file path to the registry.
	        setProspectFilePath(file);
	        setClientFilePath(file2);
